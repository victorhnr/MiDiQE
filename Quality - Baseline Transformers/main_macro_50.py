...
# Argument Quality
df_quality_train = pd.read_csv(file_path+"arg_quality_train_v77.csv", header=0, index_col=0)[["text_id", "text_tokens", "arg_quality"]]
df_quality_train = df_quality_train[df_quality_train.text_id.str.contains(target_dataset)]
df_quality_train.rename(columns={"arg_quality": "labels"}, inplace=True)
...
df_quality_val = pd.read_csv(file_path+"arg_quality_val_v77.csv", header=0, index_col=0)[["text_id", "text_tokens", "arg_quality"]]
df_quality_val = df_quality_val[df_quality_val.text_id.str.contains(target_dataset)]
df_quality_val.rename(columns={"arg_quality": "labels"}, inplace=True)
df_quality_val["labels"] = df_quality_val["labels"].replace("bad", 1)
df_quality_val["labels"] = df_quality_val["labels"].replace("good", 0)

df_quality_test = pd.read_csv(file_path+"arg_quality_test_v77.csv", header=0, index_col=0)[["text_id", "text_tokens", "arg_quality"]]
df_quality_test = df_quality_test[df_quality_test.text_id.str.contains(target_dataset)]
df_quality_test.rename(columns={"arg_quality": "labels"}, inplace=True)
...
for key in models.keys():
    ...
    for SEED in seeds:
        ...
        filepath = f"./models/Quality/{SEED}"
