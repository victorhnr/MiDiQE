...
# Argument Quality
df_quality_train = pd.read_csv(file_path+"arg_quality_train_v77.csv", header=0, index_col=0)[["text_id", "text_tokens", "arg_score"]]
df_quality_train = df_quality_train[df_quality_train.text_id.str.contains(target_dataset)]
df_quality_train["labels"] = np.where(df_quality_train["arg_score"] >= threshold, "good", "bad")
df_quality_train = df_quality_train[["text_id", "text_tokens", "labels"]]
df_quality_train["labels"] = df_quality_train["labels"].replace("bad", 1)
df_quality_train["labels"] = df_quality_train["labels"].replace("good", 0)

df_quality_val = pd.read_csv(file_path+"arg_quality_val_v77.csv", header=0, index_col=0)[["text_id", "text_tokens", "arg_score"]]
df_quality_val = df_quality_val[df_quality_val.text_id.str.contains(target_dataset)]
df_quality_val["labels"] = np.where(df_quality_val["arg_score"] >= threshold, "good", "bad")
df_quality_val = df_quality_val[["text_id", "text_tokens", "labels"]]
df_quality_val["labels"] = df_quality_val["labels"].replace("bad", 1)
df_quality_val["labels"] = df_quality_val["labels"].replace("good", 0)

df_quality_test = pd.read_csv(file_path+"arg_quality_test_v77.csv", header=0, index_col=0)[["text_id", "text_tokens", "arg_score"]]
df_quality_test = df_quality_test[df_quality_test.text_id.str.contains(target_dataset)]
df_quality_test["labels"] = np.where(df_quality_test["arg_score"] >= threshold, "good", "bad")
df_quality_test = df_quality_test[["text_id", "text_tokens", "labels"]]
df_quality_test["labels"] = df_quality_test["labels"].replace("bad", 1)
df_quality_test["labels"] = df_quality_test["labels"].replace("good", 0)
...
for key in models.keys():
    ...
    for SEED in seeds:
        ...
        filepath = f"./models/Quality/{key}_{threshold}"
