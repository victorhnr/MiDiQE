...
import re
import logging
from sklearn.metrics import f1_score, mean_squared_error, cohen_kappa_score
import pandas as pd
...
# Argument Quality
df_quality_test = pd.read_csv(file_path+"arg_quality_test_v77.csv", header=0, index_col=0)[["text_id", "text_tokens", "arg_score", "arg_quality_fine_grained", "arg_quality"]]
df_quality_test = df_quality_test[df_quality_test.text_id.str.contains("AAEC")] # or ArGPT
test_ids = df_quality_test.text_id.unique()

df_component_test = pd.read_csv(file_path+"original_components_predicted_quality_test_2_labels_complete.csv", header=0, index_col=0)[["text_id", "component_tokens", "labels", "predicted_support_quality", "predicted_attack_quality"]]
df_component_test.rename(columns={"predicted_support_quality": "supports_strenght"}, inplace=True)
df_component_test.rename(columns={"predicted_attack_quality": "attacks_strenght"}, inplace=True)
