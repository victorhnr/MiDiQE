...
print("TRAINING Relation Classification")
...
df_relation_train = pd.read_csv(file_path+"relation_train.csv", header=0, index_col=0)[["text_id", "source_tokens", "target_tokens", "labels"]]
df_relation_train = df_relation_train[df_relation_train.text_id.str.contains(target_dataset)]
df_relation_train["labels"] = df_relation_train["labels"].replace("attack", 1)
df_relation_train["labels"] = df_relation_train["labels"].replace("support", 0)
df_relation_train["labels"] = df_relation_train["labels"].replace("Against", 1)
df_relation_train["labels"] = df_relation_train["labels"].replace("For", 0)
df_relation_train = pd.merge(df_relation_train, df_component_train,  how='left', left_on=['text_id'], right_on = ['text_id'])
...
df_relation_test = pd.read_csv(file_path+"relation_test.csv", header=0, index_col=0)[["text_id", "source_tokens", "target_tokens", "labels"]]
df_relation_test = df_relation_test[df_relation_test.text_id.str.contains(target_dataset)]
df_relation_test["labels"] = df_relation_test["labels"].replace("support", 0)
df_relation_test["labels"] = df_relation_test["labels"].replace("attack", 1)
df_relation_test["labels"] = df_relation_test["labels"].replace("Against", 1)
df_relation_test["labels"] = df_relation_test["labels"].replace("For", 0)
df_relation_test = pd.merge(df_relation_test, df_component_test,  how='left', left_on=['text_id'], right_on = ['text_id'])
...
df_relation_val = pd.read_csv(file_path+"relation_val.csv", header=0, index_col=0)[["text_id", "source_tokens", "target_tokens", "labels"]]
df_relation_val = df_relation_val[df_relation_val.text_id.str.contains(target_dataset)]
df_relation_val["labels"] = df_relation_val["labels"].replace("attack", 1)
df_relation_val["labels"] = df_relation_val["labels"].replace("support", 0)
df_relation_val["labels"] = df_relation_val["labels"].replace("Against", 1)
df_relation_val["labels"] = df_relation_val["labels"].replace("For", 0)
df_relation_val = pd.merge(df_relation_val, df_component_val,  how='left', left_on=['text_id'], right_on = ['text_id'])
...
original_data_relation["val"] = val_ds

num_sup = df_relation_train[df_relation_train["labels"] == 0].shape[0]
num_atk = df_relation_train[df_relation_train["labels"] == 1].shape[0]
train_relation_weights = torch.tensor([1.0, float(num_sup/num_atk)])
...
def preprocess_function(examples):
    ...
    return tokenizer(relations, examples["text_tokens"], 
                     padding="max_length", max_length=max_lenght, truncation=True
                     )

tokenized_training_input = original_data_relation.map(preprocess_function, batched=True, remove_columns=["source_tokens", "target_tokens", "text_tokens"])
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = nn.CrossEntropyLoss(weight=train_relation_weights.to("cuda"))
...
print(relation_trainer.train_dataset)
print(relation_trainer.eval_dataset)
relation_trainer.train()

# # Make predictions
relation_results = relation_trainer.evaluate(tokenized_training_input["test"])

print(f"Relation Results: {relation_results}")


# Function to be used to create a file with the predictions
model_relation = model_relation.to('cpu')
pipe = TextClassificationPipeline(model=model_relation, tokenizer=tokenizer)
